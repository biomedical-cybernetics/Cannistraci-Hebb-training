...
def build_torch_optimizer(model, model_opt, opt):
...
def make_learning_rate_decay_fn(opt):
    ...
    if opt.decay_method == 'inoam':
        return functools.partial(
            iterative_noam_decay,
            warmup_steps=opt.warmup_steps,
            model_size=opt.rnn_size,
            update_interval=opt.update_interval,
            num_warmup_per_interval=opt.iterative_warmup_steps)
...
def iterative_noam_decay(step, warmup_steps, model_size, update_interval, num_warmup_per_interval):
    """Learning rate schedule described in
    https://arxiv.org/pdf/1706.03762.pdf.
    """
    global_lr = model_size ** (-0.5) * min(step ** (-0.5), step * warmup_steps**(-1.5))
    local_step = step % update_interval
...
class Optimizer(object):
    ...
    @classmethod
    def from_opt(cls, model, model_opt, opt, checkpoint=None):
        ...
        optimizer = cls(
            build_torch_optimizer(model, model_opt, optim_opt),
