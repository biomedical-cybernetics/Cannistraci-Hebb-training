...
def build_report_manager(opt, gpu_rank):
    if gpu_rank == 0:
        ...
        run = wandb.init(
            ...
            config={**vars(opt), "a_screen_id": os.environ.get('STY', 'Not in a screen session')},
            mode="disabled" if opt.no_log else "online",
        )
        use_wandb = True
    else:
        use_wandb = False
...
class ReportMgr(ReportMgrBase):
    def __init__(self, report_every, start_time=-1., use_wandb=False):
        """
    ...
    def maybe_log_wandb(self, stats, prefix, learning_rate, step):
        if self.use_wandb is True:
            stats.log_wandb(
                prefix, learning_rate, step)
